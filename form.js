...
if($("#ik-field").css('display') != 'none'){
	...
	if(!$("#EIAnnIncome").val().match(/^[0-9]+$/)) {
		$("#EIAnnIncome-error").html("Invalid Input!");
		output = false;
	}
...
if($("#scholarship-field").css('display') != 'none'){
	...
	if(!$("#ScholarshipIns").val().match(/^[a-zA-Z ][\w-\.]*/)) {
		$("#ScholarshipIns-error").html("Invalid Input!");
		output = false;
	}
	if(($("#ScholarshipIns").val())) {
...
if($("#AccExpen-field").css('display') != 'none'){
	...
	if(!$("#aRent").val().match(/^[0-9]*$/)) {
		$("#aRent-error").html("Invalid Input!");
		output = false;
	}
